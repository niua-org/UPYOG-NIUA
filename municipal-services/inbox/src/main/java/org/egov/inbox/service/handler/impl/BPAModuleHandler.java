...
public class BPAModuleHandler implements ModuleInboxHandler {
    ...
    private List<HashMap<String, Object>> handleBpaCitizenStatusCount(
        ...
        if (roles.contains(CITIZEN)) {
            ...
            List<Map<String, String>> tenantWiseApplns =
                    bpaService.fetchTenantWiseApplicationNumbersForCitizenInboxFromSearcher(
                            ctx.getCriteria(),
        ...
        if (!roles.contains(CITIZEN) || tenantAndApplnNumbersMap.isEmpty())
            return statusCountMap;
        ...
        if (!ctx.getStatusIdNameMap().isEmpty())
            processCriteria.setStatus(new ArrayList<>(ctx.getStatusIdNameMap().keySet()));
        ...
        for (Map.Entry<String, List<String>> entry : tenantAndApplnNumbersMap.entrySet()) {
            ...
            List<HashMap<String, Object>> tenantCount =
                    workflowService.getProcessStatusCount(ctx.getRequestInfo(), processCriteria);
            ...
            if (bpaCitizenMap.isEmpty()) {
                ...
                for (HashMap<String, Object> tenantMap : tenantCount) {
                    for (HashMap<String, Object> citizenMap : bpaCitizenMap) {
                        if (citizenMap.containsValue(tenantMap.get(STATUS_ID))) {
                            citizenMap.put(COUNT,
                                    Integer.parseInt(String.valueOf(citizenMap.get(COUNT)))
                                            + Integer.parseInt(String.valueOf(tenantMap.get(COUNT))));
                        }
                    }
                }
            }
    ...
    private List<HashMap<String, Object>> handleBpaLocalityStatusCount(
        ...
        for (Map<String, Object> statusWiseCount : statusCountMap) {
            ...
            ctx.getCriteria().getProcessSearchCriteria().setStatus(statusList);

            Integer count = bpaService.fetchApplicationCountFromSearcher(
                    ctx.getCriteria(), ctx.getStatusIdNameMap(), ctx.getRequestInfo());

            if (count == 0) statusWiseCount.clear();
            else            statusWiseCount.put(COUNT, count);
        }

        ctx.getCriteria().getProcessSearchCriteria().setStatus(inputStatuses);
