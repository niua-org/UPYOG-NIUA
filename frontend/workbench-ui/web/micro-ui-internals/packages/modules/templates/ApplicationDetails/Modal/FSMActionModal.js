...
const ActionModal = ({ t, action, tenantId, state, id, closeModal, submitAction, actionData }) => {
  ...
  useEffect(() => {
    switch (action) {
      ...
      case "SUBMIT":
      case "FSM_SUBMIT":
        return history.push(`/workbench-ui/employee/fsm/modify-application/` + applicationNumber);
      case "DECLINE":
      case "DSO_REJECT":
        //declinereason
        setFormValve(declineReason ? true : false);
      ...
      case "FSM_PAY":
        return history.push(`/workbench-ui/employee/payment/collect/FSM.TRIP_CHARGES/${applicationNumber}`);
      default:
        break;
    }
