...
import { FilterAction, Card, DetailsCard, PopUp, SearchAction } from "@upyog/workbench-ui-react-components";
...
export const ComplaintCard = ({ data, onFilterChange, onSearch, serviceRequestIdKey, searchParams }) => {
  ...
  if (data && data?.length === 0) {
    ...
    result = <DetailsCard data={data} serviceRequestIdKey={serviceRequestIdKey} linkPrefix={`/workbench-ui/employee/pgr/complaint/details/`} />;
  } else {
    result = (
      <Card style={{ marginTop: 20 }}>
