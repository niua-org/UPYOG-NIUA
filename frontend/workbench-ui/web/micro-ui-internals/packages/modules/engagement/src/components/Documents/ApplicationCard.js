...
import { Card, DetailsCard, Loader, PopUp, SearchAction, FilterAction } from "@upyog/workbench-ui-react-components";
...
const ApplicationCard = ({
  ...
  const redirectToDetailsPage = (data) => {
    ...
    if (details) {
      navigate(`/workbench-ui/employee/engagement/documents/inbox/details/${details?.name}`, { details })
    }
  }
  ...
  if (data?.length === 0) {
    result = (
      <Card style={{ marginTop: 20, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <Link className="link" to={`/workbench-ui/employee/engagement/documents/inbox/new-doc`}>{t('NEW_DOCUMENT_TEXT')}</Link>
      </Card>
    );
  }
