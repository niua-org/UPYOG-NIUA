...
const WorkflowActions = ({ businessService, tenantId, applicationNo, forcedActionPrefix, ActionBarStyle = {}, MenuStyle = {}, applicationDetails, url, setStateChanged, moduleCode,editApplicationNumber,editCallback ,callback}) => {
  ...
  const onActionSelect = (action) => {
    ...
    if(bsEstimate === businessService && action?.action === "RE-SUBMIT"){
        navigate(`/workbench-ui/employee/estimate/create-estimate?tenantId=${tenantId}&projectNumber=${editApplicationNumber}&estimateNumber=${applicationDetails?.estimateNumber}&isEdit=true`);
        return 
    }

    if(bsContract === businessService && action?.action === "EDIT"){
      navigate(`/workbench-ui/employee/contracts/create-contract?tenantId=${tenantId}&workOrderNumber=${applicationNo}`);
      return 
  }
    ...
    if(bsPurchaseBill === businessService && action?.action==="RE-SUBMIT"){
      navigate(`/workbench-ui/employee/expenditure/create-purchase-bill?tenantId=${tenantId}&billNumber=${editApplicationNumber}`);
      return 
    }
