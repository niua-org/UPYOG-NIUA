...
const EditVendor = ({ parentUrl, heading }) => {
    ...
    if (dsoData && dsoData[0]) {
      ...
      let values = {
        ...
        selectGender: dsoDetails?.dsoDetails?.owner?.gender
          ? { code: dsoDetails?.dsoDetails?.owner?.gender, i18nKey: dsoDetails?.dsoDetails?.owner?.gender }
          : null,
        ...
        serviceType: dsoDetails?.dsoDetails?.additionalDetails?.serviceType
          ? {
              code: dsoDetails?.dsoDetails?.additionalDetails?.serviceType,
              i18nKey: dsoDetails?.dsoDetails?.additionalDetails?.serviceType,
              value: dsoDetails?.dsoDetails?.additionalDetails?.serviceType,
            }
          : null,
  ...
  const onSubmit = (data) => {
    const name = data?.vendorName || dsoDetails?.name;
    const phone = data?.phone || dsoDetails?.owner?.mobileNumber;
    const pincode = data?.pincode;
    const street = data?.street?.trim();
    const doorNo = data?.doorNo?.trim();
    const plotNo = data?.plotNo?.trim();
    const landmark = data?.landmark?.trim();
    const city = data?.address?.city?.name || dsoDetails?.address?.city;
    const state = data?.address?.city?.state || dsoDetails?.address?.state;
    const district = data?.address?.city?.name || dsoDetails?.address?.district;
    const region = data?.address?.city?.name || dsoDetails?.address?.region;
    const buildingName = data?.buildingName?.trim();
    const localityCode = data?.address?.locality?.code || dsoDetails?.address?.locality?.code;
    const localityName = data?.address?.locality?.name || dsoDetails?.address?.locality?.name;
    const localityArea = data?.address?.locality?.area || dsoDetails?.address?.locality?.area;
    const additionalDetails = data?.additionalDetails;
    const serviceType = data?.serviceType?.code || dsoDetails?.additionalDetails?.serviceType;
    const gender = data?.selectGender?.code || data?.selectGender;
    const emailId = data?.emailId;
    const dob = data?.dob ? new Date(`${data.dob}`).getTime() : dsoDetails?.owner?.dob;
    const formData = {
      vendor: {
        ...dsoDetails,
        name,
    ...
    mutate(formData, {
      ...
      onSuccess: (data, variables) => {
        ...
        setTimeout(() => {
          closeToast();
          navigate(`/upyog-ui/employee/vendor/registry/vendor-details/${dsoId}`);
        }, 3000);
      },
    });
  };
