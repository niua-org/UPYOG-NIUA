...
const VendorPincode = ({ t, config, onSelect, formData = {}, userType, register, errors, setError, formState, clearErrors }) => {
  ...
  const inputs = [
    {
      ...
      validation: {
        ...
        pattern: "[0-9]+",
        max: "9999999",
        // title: t("VENDOR_ADDRESS_PINCODE_INVALID"),
      },
    },
  ];
  ...
  function onChange(e) {
    ...
    if (userType === "employee") {
      ...
      if (validPincode) {
        ...
        if (!foundValue) setLocalError(t("VENDOR_COMMON_PINCODE_NOT_SERVICABLE"));
      }
  ...
  const goNext = async (data) => {
    ...
    if (foundValue) {
      ...
      setPincodeServicability("VENDOR_COMMON_PINCODE_NOT_SERVICABLE");
    }
  };
