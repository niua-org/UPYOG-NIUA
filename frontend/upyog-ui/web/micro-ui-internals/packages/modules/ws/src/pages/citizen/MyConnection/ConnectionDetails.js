...
const ConnectionDetails = () => {
  ...
  const getAddress = address => {
    return `${address?.doorNo ? `${address?.doorNo}, ` : ""}${address?.street ? `${address?.street}, ` : ""}${address?.landmark ? `${address?.landmark}, ` : ""}${address?.locality?.code ? t(`TENANTS_MOHALLA_${address?.locality?.code}`) : ""}${address?.city?.code || address?.city ? `, ${t(address?.city.code || address?.city)}` : ""}${address?.pincode ? `, ${address.pincode}` : " "}`;
  };
  let filter1 = {
    tenantId: tenantId,
  ...
  let filter2 = {
    tenantId: tenantId,
    connectionNumber: applicationNobyData?.includes("SW") ? data?.SewerageConnections?.[0]?.connectionNo : data?.WaterConnection?.[0]?.connectionNo
  };
  var {
    ...
    filters: filter2,
    privacy: Digit.Utils.getPrivacyObject(),
    enabled: filter2?.connectionNumber ? true : false
  }, true);
  ...
  const {
    ...
    select: data => data["common-masters"]?.uiCommonPay?.filter(({
      code
    }) => "WS"?.includes(code))[0]?.receiptKey || "consolidatedreceipt"
  });
  async function getConnectionDetail({
    tenantId
  }, order, mode = "download") {
    let requestData = {
      ...state
  ...
  const printApplicationReceipts = async () => {
    ...
    const payments = await Digit.PaymentService.getReciept(tenantId, "WS", {
      consumerCodes: data?.WaterConnection?.[0]?.connectionNo
    });
    let response = {
  ...
  const fetchBillParams = {
    consumerCode: state?.connectionNo
  };
