...
const WSDisconnectionForm = ({
  ...
  } = Digit.Hooks.ws.useMDMS(stateCode, "ws-services-masters", ["DisconnectionReason"]);
  const {
    ...
    return <div>
        ...
        <FormStep config={config} onSelect={handleSubmit} onSkip={onSkip} t={t}>
          ...
          <RadioButtons t={t} options={disconnectionTypeList} optionsKey="i18nKey" value={disconnectionData.type?.value?.code} selectedOption={disconnectionData.type?.value} isMandatory={false} onSelect={val => filedChange({
            ...
            <InfoIcon />
            <span className="tooltiptext" style={citizenTooltipStyle}>
                   {t("SHOULD_BE_DATE") + " " + slaData?.slaDays + " " + t("DAYS_OF_APPLICATION_DATE")}
                  </span>
            </div>
          </CardLabel>
          <div className="field">
          <DatePicker date={disconnectionData?.date} onChange={date => {
          ...
          <div className="field">
          <DatePicker date={disconnectionData?.endDate} onChange={date => {
                setDisconnectionData({
                  ...disconnectionData,
                  endDate: date
                });
    ...
    <FormStep config={config} onSelect={handleEmployeeSubmit} onSkip={onSkip} t={t}>
      ...
      <CardSectionHeader>
        {t("WS_DISCONNECTION_TYPE") + "*"}
            <div className={`tooltip`}>
            <InfoIcon />
            <span className="tooltiptext" style={employeeDisconnectionTypeTooltipStyle}>
        ...
        <RadioButtons t={t} options={disconnectionTypeList} optionsKey="i18nKey" value={disconnectionData.type?.value?.code} selectedOption={disconnectionData.type?.value} isMandatory={false} onSelect={val => filedChange({
          ...
          <CardLabel className="card-label-smaller ws-auto-95">
            ...
            <div className={`tooltip`}>
            <InfoIcon />
            <span className="tooltiptext" style={employeeDateTooltipStyle}>
                    {t("SHOULD_BE_DATE") + " " + slaData?.slaDays + " " + t("DAYS_OF_APPLICATION_DATE")}
                  </span>
            </div>
          </CardLabel>
...
function SelectDocument({
  ...
  const [selectedDocument, setSelectedDocument] = useState(filteredDocument ? {
    ...
    i18nKey: filteredDocument?.documentType
  ...
  return <div className="ws-auto-103">
          ...
          <UploadFile id={`noc-doc-1-${key}`} extraStyleName={"propertyCreate"} accept="image/*, .pdf, .png, .jpeg, .jpg" onUpload={selectfile} onDelete={() => {
          setUploadedFile(null);
          setCheckRequiredFields(true);
        }} message={uploadedFile ? `1 ${t(`CS_ACTION_FILEUPLOADED`)}` : t(`ES_NO_FILE_SELECTED_LABEL`)} error={error} />
          </div>
          </LabelFieldPair>
