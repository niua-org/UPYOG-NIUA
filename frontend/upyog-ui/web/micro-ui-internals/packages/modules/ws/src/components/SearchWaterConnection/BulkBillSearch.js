    ...
    const promise = new Promise((resolve, reject) => {
      ...
      fileReader.onload = e => {
        ...
        const meterReadingListFilter = data.map(meter => {
          return {
            "billingPeriod": meter.billingPeriod,
            "currentReading": meter.currentReading,
            "currentReadingDate": meter.currentReadingDate,
            "lastReading": meter.lastReading,
            "lastReadingDate": meter.lastReadingDate,
            "connectionNo": meter.connectionNo,
        ...
        const rejectedReading = meterReadingListFilter.filter(element => !meterReadingList.includes(element));
  ...
  const handleBulkSubmit = async () => {
    if (meterReadingMutation) {
      ...
      let meterReadingsPayload = {
        meterReadingList: meterReadingData
      };
      await meterReadingMutation(meterReadingsPayload, {
        ...
        onSuccess: async (data, variables) => {
          ...
          setShowToast({
            key: "success",
            label: "WS_METER_READING_ADDED_SUCCESFULLY"
          });
    ...
    : resultOk ? <div className="ws-auto-13">
      ...
      <div>
        ...
        width: "auto"
            ...
            }} onPageSizeChange={onPageSizeChange} currentPage={getValues("offset") / getValues("limit")} onNextPage={nextPage} onPrevPage={previousPage} pageSizeLimit={getValues("limit")} onSort={onSort} disableSort={true} sortParams={[{
              id: getValues("sortBy"),
              desc: getValues("sortOrder") === "DESC" ? true : false
            }]} />
