...
const ApplicationBillAmendment = () => {
  ...
  let billSearchData = preBillSearchData?.filter(e => availableBillAmendmentTaxHeads?.find(taxHeadMaster => taxHeadMaster.code === e.taxHeadCode));
    ...
    try {
      if (data?.target?.files?.[0]?.size < 5242880 && /(.*?)(jpg|jpeg|png|pdf)$/i.test(data?.target?.files?.[0]?.type)) {
        ...
        if (fileStoreId) {
          const {
            data: urlResponse
          } = await Digit.UploadServices.Filefetch([fileStoreId], tenantId);
  ...
  const getDemandDetailsComparingUpdates = d => {
    ...
    if (servicev1 === "WS") actionPerformed = JSON.parse(JSON.stringify(d?.WS_REDUCED_AMOUNT?.VALUE ? d?.WS_REDUCED_AMOUNT : d?.WS_ADDITIONAL_AMOUNT));else actionPerformed = JSON.parse(JSON.stringify(d?.SW_REDUCED_AMOUNT?.VALUE ? d?.SW_REDUCED_AMOUNT : d?.SW_ADDITIONAL_AMOUNT));
  ...
  const onFormSubmit = d => {
    ...
    if (state?.data?.action === "RE-SUBMIT-APPLICATION") {
      ...
      const AmendmentUpdate = {
        ...state?.data?.applicationDetails?.applicationData?.billAmendmentDetails,
        workflow,
        ...data?.Amendment
      };
      mutate({
  ...
  function getReasonDocNoHeader() {
    if (amendmentReason?.code === "COURT_CASE_SETTLEMENT") return "WS_COURT_ORDER_NO";else if (amendmentReason?.code === "ARREAR_WRITE_OFF" || amendmentReason?.code === "ONE_TIME_SETTLEMENT") return "WS_GOVERNMENT_NOTIFICATION_NUMBER";else return "WS_DOCUMENT_NO";
  }
  let adhocAmount = 0;
  ...
  return <form onSubmit={handleSubmit(onFormSubmit)}>
      ...
      <Card>
        ...
        {!isBillSearchLoading ? <table cellPadding={"8px"} cellSpacing={"10px"}>
          ...
          maxWidth: "50%"
            ...
            <tr className="ws-auto-263">
              ...
              <th>
                <>
                  <Controller name={`${servicev1}_ADDITIONAL_AMOUNT.VALUE`} key={`${servicev1}_ADDITIONAL_AMOUNT.VALUE`} control={control} rules={{
                    validate: value => {
        ...
        {BillAmendmentMDMSLoading ? <Loader /> : <>
            ...
            return <Dropdown style={isMobile && isEmployee ? {} : {
              width: "640px"
            ...
            <LabelFieldPair>
              ...
              width: "640px"
            }} name="reasonDocumentNumber" inputRef={register({
              required: true
            })} />
              </div>
            </LabelFieldPair>
        ...
        {requiredDocuments?.map(e => <div className="ws-auto-296">
          <LabelFieldPair>
            ...
            <div className="field">
              <Controller name={`DOCUMENTS.${e?.documentType}`} control={control} rules={e?.required ? {
              required: true
            } : {}} render={props => <UploadFile id={`ws-doc-${e.documentType}`} onUpload={d => functionToHandleFileUpload(d, e?.documentType, props)} onDelete={() => dispatch({
              type: "remove",
              payload: {
                id: e?.documentType
              }
            ...
            message={e?.url ? `1 ${t(`CS_ACTION_FILEUPLOADED`)}` : t(`CS_ACTION_NO_FILEUPLOADED`)} accept="image/*, .pdf, .png, .jpeg, .jpg" textStyles={{
              width: "100%"
            }} inputStyles={{
              width: "280px"
            }} error={error?.message} />} />
