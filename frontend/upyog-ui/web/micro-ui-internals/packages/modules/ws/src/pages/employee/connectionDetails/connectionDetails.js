...
const GetConnectionDetails = () => {
  ...
  const {
    ...
    consumerCode: applicationDetails?.applicationData?.connectionNo,
  ...
  const checkifPrivacyenabled = Digit.Hooks.ws.useToCheckPrivacyEnablement({
    privacy: {
      uuid: applicationDetails?.applicationData?.applicationNo?.includes("WS") ? applicationDetails?.applicationData?.WaterConnection?.[0]?.connectionHolders?.[0]?.uuid : applicationDetails?.applicationData?.SewerageConnections?.[0]?.connectionHolders?.[0]?.uuid,
  ...
  async function getBillSearch() {
    if (applicationDetails?.fetchBillsData?.length > 0) {
      ...
      const wsConnectionDetails = await Digit.WSService.search({
        tenantId,
