...
const WSRoadCuttingDetails = ({
  ...
  const {
    isMdmsLoading,
    data: mdmsData
  } = Digit.Hooks.ws.useMDMS(stateCode, window.location.href.includes("sewerage") ? "sw-services-calculation" : "ws-services-calculation", ["RoadType"]);
  ...
  const removeroadCutt = roadCutt => {
    roadCutt?.id !== null ? setRoadCuttingDetails(prev => prev.map(e => e.key == roadCutt.key && e.id !== null ? {
      ...roadCutt,
      id: e.id,
      status: "INACTIVE"
    } : {
