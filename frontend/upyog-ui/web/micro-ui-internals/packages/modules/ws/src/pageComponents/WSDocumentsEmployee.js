...
const WSDocumentsEmployee = ({
  ...
  return <div>
      ...
      return <SelectDocument key={index} document={document} action={action} t={t} id={`pt-document-${index}`} error={error} setError={setError} setDocuments={setDocuments} documents={documents} formData={formData} setFormError={setFormError} clearFormErrors={clearFormErrors} config={config} formState={formState} />;
    })}
...
function SelectDocument({
  ...
  return <div className="ws-auto-129">
      {doc?.hasDropdown ? <LabelFieldPair>         
          ...
          <Dropdown id={`doc-${doc?.code}`} key={`doc-${doc?.code}`} className="form-field" selected={selectedDocument ? selectedDocument : filteredDocument ? filteredDocument : selectedDocument} option={dropDownData.map(e => ({
        ...e,
