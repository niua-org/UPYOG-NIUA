...
const ModifyApplicationDetails = () => {
  ...
  if (workflowDetails?.data?.nextActions?.length > 0 && workflowDetails?.data?.actionState?.nextActions?.length > 0 && !workflowDetails?.data?.actionState?.nextActions?.find(e => e.action === "EDIT") && !workflowDetails?.data?.actionState?.nextActions?.find(e => e.action === "RESUBMIT_APPLICATION") && !workflowDetails?.data?.actionState?.nextActions?.find(e => e.action === "ACTIVATE_CONNECTION") && !workflowDetails?.data?.actionState?.nextActions?.find(e => e.action === "SUBMIT_APPLICATION")) {
  ...
  const handleDownloadPdf = async () => {
    ...
    const PDFdata = getModifyPDFData({
      ...result
    }, {
