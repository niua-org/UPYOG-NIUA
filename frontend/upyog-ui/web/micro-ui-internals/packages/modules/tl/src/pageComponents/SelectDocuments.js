...
const TLSelectDocuments = ({ t, config, onSelect, userType, formData, setError: setFormError, clearErrors: clearFormErrors, formState }) => {
  ...
  const goNext = () => {
    onSelect(config.key, {
      documents,
      propertyTaxDocumentsLength: propertyTaxDocuments?.length
    });
  };
  useEffect(() => {
    goNext();
  ...
  return <div>
      ...
      //   }
      // }
      return <SelectDocument key={index} document={document} action={action} t={t} error={error} setError={setError} setDocuments={setDocuments} documents={documents} formData={formData} setFormError={setFormError} clearFormErrors={clearFormErrors} config={config} formState={formState} />;
    })}
...
function SelectDocument({
  ...
  if (dropdownFilter) {
    ...
    if (action === "create") {
      ...
      if (value) {
        if (!onArray) dropDownData = dropdownData.filter(e => e.parentValue.includes(value));else {
  ...
  return <div className="tl-auto-40">
      ...
      <LabelFieldPair>
        ...
        <div className="field">
          <UploadFile id={"tl-doc"} onUpload={selectfile} onDelete={() => {
          setUploadedFile(null);
        }} message={uploadedFile ? `1 ${t(`CS_ACTION_FILEUPLOADED`)}` : t(`CS_ACTION_NO_FILEUPLOADED`)} textStyles={{
          width: "100%"
