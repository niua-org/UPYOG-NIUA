...
const SelectOwnershipProof = ({
  ...
  return <React.Fragment>
    ...
    <FormStep config={config} onSelect={handleSubmit} onSkip={onSkip} t={t} isDisabled={!uploadedFile || error} isMandatory={isMandatory}>
      <CardLabelDesc className="tl-auto-55">{t(`TL_UPLOAD_OWNERSHIP_RESTRICTIONS_TYPES`)}</CardLabelDesc>
      <CardLabelDesc className="tl-auto-56">{t(`TL_UPLOAD_RESTRICTIONS_SIZE`)}</CardLabelDesc>
      ...
      {error ? <div className="tl-auto-57">{error}</div> : ""}
      <div className="tl-auto-58"></div>
    </FormStep>
    </React.Fragment>;
};
