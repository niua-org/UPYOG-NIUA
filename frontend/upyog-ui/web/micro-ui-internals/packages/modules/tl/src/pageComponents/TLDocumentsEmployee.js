...
const TLDocumentsEmployee = ({
  ...
  return <div>
      ...
      return <SelectDocument key={index} document={document} action={action} t={t} id={`tl-doc-${index}`} error={error} setError={setError} setDocuments={setDocuments} documents={documents} formData={formData} setFormError={setFormError} clearFormErrors={clearFormErrors} config={config} formState={formState} />;
    })}
    ...
    if (doc && formData?.documents?.documents?.length > 0) {
      for (let i = 0; i < formData?.documents?.documents?.length; i++) {
        if (doc?.documentType === formData?.documents?.documents?.[i]?.documentType) {
          setSelectedDocument({
            documentType: formData?.documents?.documents?.[i]?.documentType,
            id: formData?.documents?.documents?.[i]?.id
          });
