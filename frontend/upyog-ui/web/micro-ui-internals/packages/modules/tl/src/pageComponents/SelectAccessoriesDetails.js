...
const SelectAccessoriesDetails = ({
  ...
  const [fields, setFeilds] = useState(formData?.TradeDetails && formData?.TradeDetails?.accessories && formData?.TradeDetails?.accessories.length > 0 ? formData?.TradeDetails?.accessories : [{
    accessory: "",
    accessorycount: "",
    unit: null,
    uom: null
  ...
  function getAccessoryCategoryDropDown() {
    ...
    Data && Data?.TradeLicense?.AccessoriesCategory.map(ob => {
  ...
  function canMoveNext() {
    if (!fields?.[0]?.accessory || !fields?.[0]?.accessorycount || fields?.[0]?.unit && !fields?.[0]?.uom || AccCountError || AccUOMError) return true;else return false;
  }
      ...
      {isLoading ? <Loader /> : <FormStep config={config} onSelect={goNext} onSkip={onSkip} t={t} forcedError={t(AccCountError) || t(AccUOMError)} isDisabled={canMoveNext()}>
        ...
        return <div key={`${field}-${index}`}>
            ...
            options={sortDropdownNames(accessories, "i18nKey", t) || []} selectedOption={field.accessory} onSelect={e => selectAccessory(index, e)} isPTFlow={true} /> : <Loader />}
                  ...
                  <TextInput t={t} type={"text"} isMandatory={false} optionKey="i18nKey" name="AccessoryCount" value={field.accessorycount} onChange={e => selectAccessoryCount(index, e)}
            //disable={(isEditTrade || isRenewTrade) && (formData?.TradeDetails?.accessories.length - 1 < index ? false : field.accessorycount)}
            disable={isRenewTrade || isEditTrade ? !enableUOM : false} {...validation = {
              isRequired: true,
              pattern: "[0-9]+",
              type: "text",
