...
const TLApplicationDetails = () => {
  ...
  const {
    ...
    tenantId,
    filters: {
      propertyIds: application?.[0]?.tradeLicenseDetail?.additionalDetail?.propertyId
    }
  }, {
  ...
  const dowloadOptions = paymentsHistory?.Payments?.length > 0 && application?.[0]?.status !== "EXPIRED" && application?.[0]?.status !== "CANCELLED" && application?.[0]?.status !== "PENDINGPAYMENT" && application?.[0]?.status !== "MANUALEXPIRED" ? [{
