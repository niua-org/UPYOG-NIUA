...
const SelectTradeUnitsInitial = ({
  ...
  const [fields, setFeilds] = useState(formData?.TradeDetails && formData?.TradeDetails?.units || [{
    tradecategory: "",
    tradetype: "",
    tradesubtype: "",
    unit: null,
  ...
  billingSlabTradeTypeData && billingSlabTradeTypeData.length > 0 && billingSlabTradeTypeData.filter(e => e.structureType === formData?.TradeDetails?.BuildingType?.code.toString())?.map(ob => {
  ...
  billingSlabTradeTypeData && billingSlabTradeTypeData.length > 0 && billingSlabTradeTypeData.filter(e => e.structureType === formData?.TradeDetails?.VehicleType?.code.toString())?.map(ob => {
    if (!TradeCategoryMenu.some(TradeCategoryMenu => TradeCategoryMenu.code === `${ob.tradeType.split(".")[0]}`)) {
      TradeCategoryMenu.push({
        i18nKey: `TRADELICENSE_TRADETYPE_${ob.tradeType.split(".")[0]}`,
        code: `${ob.tradeType.split(".")[0]}`
  ...
  function selectTradeSubType(i, value) {
    ...
    value && billingSlabTradeTypeData && billingSlabTradeTypeData?.length > 0 && billingSlabTradeTypeData.filter(e => e.structureType === formData?.TradeDetails?.BuildingType?.code.toString())?.map(ob => {
    ...
    value && billingSlabTradeTypeData && billingSlabTradeTypeData?.length > 0 && billingSlabTradeTypeData.filter(e => e.structureType === formData?.TradeDetails?.VehicleType?.code.toString())?.map(ob => {
      if (value.code === ob.tradeType) {
        units[i].unit = ob.uom;
        setUnitOfMeasure(ob.uom);
        // setFeilds(units);
  ...
  return <React.Fragment>
      ...
      {isLoading ? <Loader /> : <FormStep config={config} onSelect={goNext} onSkip={onSkip} t={t} forcedError={t(error)} isDisabled={!fields[0].tradecategory || !fields[0].tradetype || !fields[0].tradesubtype}>
          {fields?.map((field, index) => {
        return <div key={`${field}-${index}`}>
                <div className="tl-auto-73">
                  ...
                  {!isLoading ? <RadioButtons t={t} options={TradeCategoryMenu} optionsKey="i18nKey" name={`TradeCategory-${index}`} value={field?.tradecategory} selectedOption={field?.tradecategory} onSelect={e => selectTradeCategory(index, e)} labelKey="" isPTFlow={true} /> : <Loader />}
                  <CardLabel>{`${t("TL_NEW_TRADE_DETAILS_TRADE_TYPE_LABEL")}`}<span className="check-page-link-button"> *</span></CardLabel>
                  <Dropdown t={t} optionKey="i18nKey" isMandatory={config.isMandatory} option={getTradeTypeMenu(field?.tradecategory)} selected={field?.tradetype} select={e => selectTradeType(index, e)} />
                  <CardLabel>{`${t("TL_NEW_TRADE_DETAILS_TRADE_SUBTYPE_LABEL")}`}<span className="check-page-link-button"> *</span></CardLabel>
                    <Dropdown t={t} optionKey="i18nKey" isMandatory={config.isMandatory} option={getTradeSubTypeMenu(field?.tradetype)} selected={field?.tradesubtype} optionCardStyles={{
              maxHeight: "125px",
              overflow: "scroll"
            ...
            //value={UnitOfMeasure}
            value={field?.unit} onChange={e => selectUnitOfMeasure(index, e)} disable={true} className="tl-auto-76" />
            ...
            //value={UomValue}
            value={field?.uom} onChange={e => selectUomValue(index, e)} disable={!field.unit} {...validation = {
              isRequired: true,
              pattern: "[0-9]+",
              type: "text",
              title: t("TL_WRONG_UOM_VALUE_ERROR")
            }} className="tl-auto-77" />
                </div>
              </div>;
      })}
          <div className="tl-auto-78">
            <button type="button" onClick={() => handleAdd()} className="tl-auto-79">
