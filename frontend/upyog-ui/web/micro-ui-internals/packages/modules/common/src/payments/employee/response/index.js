...
import { Banner, Card, CardText, SubmitBar, ActionBar, DownloadPrefixIcon, Loader, Menu, LinkLabel } from "@nudmcdgnpm/digit-ui-react-components";
...
export const SuccessfulPayment = (props) => {
  ...
  const printPermissionLetter = async () => {
    const applicationDetails = await Digit.CHBServices.search({
      tenantId,
  ...
  const printADSPermissionLetter = async () => {
    const applicationDetails = await Digit.ADSServices.search({
      tenantId,
      filters: { bookingNo: consumerCode }
    });
