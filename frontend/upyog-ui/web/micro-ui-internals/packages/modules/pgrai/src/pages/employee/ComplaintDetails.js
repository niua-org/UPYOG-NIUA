 ...
 * 
 * This component displays detailed information about a citizen complaint including:
 * - Complaint summary information
 * - Supporting images/documents uploaded by citizens
 * - Timeline of complaint status changes and actions
 * - Workflow actions for officials to process the complaint (assign, reassign, resolve, reject)
 * 
 * The component allows officials to:
 * - View complete complaint information
 * - See the workflow history with comments and attachments
 * - Take actions on complaints with supporting comments and files
 * - Track the complaint through its lifecycle
 * 
 * Part of a larger complaint management system for government/municipal services.
 */
...
const ComplaintDetailsModal = ({ workflowDetails, complaintDetails, close, popup, selectedAction, onAssign, tenantId, t }) => {
  ...
  const useEmployeeData = Digit.Hooks.pgrAi.useFilterEmployee(
    tenantId, 
...
export const ComplaintDetails = (props) => {
  ...
  const { isLoading, complaintDetails, revalidate: revalidateComplaintDetails } = Digit.Hooks.pgrAi.useApplicationDetails({ tenantId, id });
  const workflowDetails = Digit.Hooks.useWorkflowDetails({ tenantId, id, moduleCode: "PGRAI", role: "EMPLOYEE" });
  ...
  async function onAssign(selectedEmployee, comments, uploadedFile) {
    setPopup(false);
    const response = await Digit.PGRAIUpdate.assign(complaintDetails, selectedAction, selectedEmployee, comments, uploadedFile, tenantId);
