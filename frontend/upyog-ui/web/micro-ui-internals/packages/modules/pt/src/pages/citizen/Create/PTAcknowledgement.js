...
const PTAcknowledgement = ({ data, onSuccess }) => {
  ...
  return mutation.isPending || mutation.isIdle ? (
    ...
    <Card>
      ...
      {/* {mutation.isSuccess && <Link to={`/upyog-ui/citizen/feedback?redirectedFrom=&propertyId=${mutation.isSuccess ? mutation?.data?.Properties[0]?.propertyId : ""}&acknowldgementNumber=${mutation.isSuccess ? mutation?.data?.Properties[0]?.acknowldgementNumber : ""}&creationReason=${mutation.isSuccess ? mutation?.data?.Properties[0]?.creationReason : ""}&tenantId=${mutation.isSuccess ? mutation?.data?.Properties[0]?.tenantId : ""}&locality=${mutation.isSuccess ? mutation?.data?.Properties[0]?.address?.locality?.code : ""}`}>
