...
const formatUnits = (units = [], currentFloor, isFloor, formData) => {
  ...
  return units.map((unit, index) => {
    ...
    return {
      ...
      rentedMonths: rentedMonths.find(month => month.code === formData?.additionalDetails?.unit?.[index]?.RentedMonths) || null,
      nonRentedMonthsUsage: nonRentedMonthsUsage.find(month => month.code === formData?.additionalDetails?.unit?.[index]?.NonRentedMonthsUsage) || null,
      ...
      occupancyType: unit?.occupancyType ? {
        ...
        i18nKey: `PROPERTYTAX_OCCUPANCYTYPE_${unit?.occupancyType}`
      } : "",
      floorNo: unit?.floorNo || Number.isInteger(unit?.floorNo) ? {
        code: unit.floorNo,
        i18nKey: `PROPERTYTAX_FLOOR_${unit?.floorNo}`
      } : {},
      unitType: unit?.unitType ? {
        code: unit.unitType,
        i18nKey: `PROPERTYTAX_BILLING_SLAB_${unit?.unitType?.code || unit?.unitType}`
      } : ""
    };
  });
};
const SelectPTUnits = React.memo(({
  ...
  const [fields, setFields] = useState(formatUnits(isFloor ? formData?.units?.filter(ee => ee.floorNo == currentFloor) : formData?.units, currentFloor, isFloor, formData));
  ...
  const {
    data: mdmsData,
    isLoading
  } = Digit.Hooks.useCommonMDMSV2(Digit.ULBService.getStateId(), "PropertyTax", ["Floor", "OccupancyType", "UsageCategory"], {
    select: data => {
      let usageCategory = data?.PropertyTax?.UsageCategory?.map(category => getUsageCategory(category.code)).filter(category => category.usageCategoryDetail === false && category.usageCategorySubMinor === false && category.usageCategoryMinor !== false).map(category => ({
        code: category.usageCategoryMinor,
        i18nKey: `PROPERTYTAX_BILLING_SLAB_${category.usageCategoryMinor}`
      }));
      let subCategory = Digit.Utils.getUnique(data?.PropertyTax?.UsageCategory.map(e => getUsageCategory(e.code)).filter(e => e.usageCategoryDetail).map(e => ({
    ...
    let unitsdata = fields.map(field => {
      ...
      Object.keys(field).filter(key => field[key]).map(key => {
        ...
        if (key === "usageCategory") {
          unit["usageCategory"] = mdmsData?.usageDetails.find(e => {
            const splitCode = e.code.split(".")[0];
            if (field[key]?.code === "RESIDENTIAL") {
              return splitCode === field[key]?.code && e.code.includes(typeof field["unitType"] === "object" ? field["unitType"]?.code : field["unitType"]);
            } else {
              return e.code.includes(field[key]?.code) && e.code.includes(typeof field["unitType"] === "object" ? field["unitType"]?.code : field["unitType"]);
            }
          })?.code;
  ...
  function isAllowedNext() {
    ...
    fields && fields?.map(ob => {
      if (!ob?.usageCategory || Object.keys(ob?.usageCategory) == 0 || !ob?.occupancyType || !ob?.builtUpArea /* || (!(ob?.floorNo)|| Object.keys(ob?.floorNo) == 0 )*/) valueNotthere = 1;else if (!(ob?.usageCategory?.code === "RESIDENTIAL") && !ob?.unitType) valueNotthere = 1;else if (ob?.occupancyType?.code === "RENTED" && !ob?.arv) valueNotthere = 1;
    });
    ...
    <FormStep config={(config.texts.header = getheader(), config)} onSelect={goNext} onSkip={onSkip} t={t} isDisabled={isAllowedNext()}>
      {fields.map((field, index) => {
        return <div key={`${field}-${index}`}>
            <div className="pt-auto-83">
            ...
            }]} selected={field?.usageCategory} select={e => selectUsageCategory(index, e)} />
              {field?.usageCategory?.code && field.usageCategory.code.includes("RESIDENTIAL") === false && <>
                  ...
                  <div className={"form-pt-dropdown-only"}>
                    <Dropdown t={t} optionKey="i18nKey" isMandatory={config.isMandatory} option={mdmsData?.UsageSubCategory?.filter(category => category.usageCategoryMinor === field?.usageCategory?.code)} selected={field?.unitType} select={e => selectSubUsageCategory(index, e)} />
                  </div>
                </>}
              <CardLabel>{`${t("PT_FORM2_OCCUPANCY")}`}<span className="check-page-link-button"> *</span></CardLabel>
              <div className={"form-pt-dropdown-only"}>
                <Dropdown t={t} optionKey="i18nKey" isMandatory={config.isMandatory} option={mdmsData?.OccupancyType} selected={field?.occupancyType} select={e => selectOccupancy(index, e)} />
              </div>
              {field?.occupancyType?.code && field.occupancyType.code.includes("RENTED") && <>
                  ...
                  <TextInput t={t} type={"text"} isMandatory={false} optionKey="i18nKey" name="arv" value={field?.arv} onChange={e => onChangeRent(index, e)} {...{
                isRequired: true,
                pattern: "[0-9]+",
                type: "text",
              ...
              <div className={"form-pt-dropdown-only"}>
                <Dropdown t={t} optionKey="i18nKey" isMandatory={config.isMandatory} option={rentedMonths} selected={field?.rentedMonths} select={e => selectrentedMonths(index, e)} />
              </div>
              ...
              <div className={"form-pt-dropdown-only"}>
                <Dropdown t={t} optionKey="i18nKey" isMandatory={config.isMandatory} option={nonRentedMonthsUsage} selected={field?.nonRentedMonthsUsage} select={e => selectnonRentedMonthsUsage(index, e)} />
              </div>                
                </>} </>}
              ...
              <TextInput t={t} type={"text"} isMandatory={false} optionKey="i18nKey" name="builtUpArea" value={field?.builtUpArea || ""} onChange={e => onChangeArea(index, e)} {...{
              isRequired: true,
              pattern: "[0-9]+",
              type: "text",
              title: t("CORE_COMMON_REQUIRED_ERRMSG")
              ...
              {!isFloor && <>
                  ...
                  <div className={"form-pt-dropdown-only"}>
                    <Dropdown t={t} optionKey="i18nKey" isMandatory={config.isMandatory} option={mdmsData?.Floor} selected={field?.floorNo} select={e => selectFloor(index, e)} />
                  </div>
                </>}
            </div>
          </div>;
