  ...
  return <React.Fragment>
      <div className="filter">
        <div className="filter-card">
          <div className="heading pt-auto-19">
            <div className="filter-label pt-auto-20">
              <span>
              ...
              <span className="pt-auto-21">{t("ES_COMMON_FILTER_BY")}:</span>
            </div>
            <div className="clearAll" onClick={clearAll}>
              {t("ES_COMMON_CLEAR_ALL")}
            </div>
            {props.type === "desktop" && <span className="clear-search pt-auto-22" onClick={clearAll}>
          ...
          <div>
            ...
            uuid: d
          })} selectedOption={_searchParams?.uuid} t={t} optionsKey="name" options={[{
            code: "ASSIGNED_TO_ME",
            ...
            <div>
              <div className="filter-label pt-auto-23">
                {t("ES_INBOX_LOCALITY")}:
              </div>
              ...
              <div className="tag-container">
                ...
                return <RemoveableTag key={index} text={t(locality.i18nkey)} onClick={() => {
                  localParamChange({
            ...
            <div>
              <div className="filter-label pt-auto-24">
                {t("ES_PT_APP_TYPE")}
              </div>
              {ApplicationTypeMenu.map((e, index) => {
              const checked = _searchParams?.services?.includes(e.value);
              return <CheckBox key={index + "service"} label={t(e.label)} value={e.label} checked={checked} onChange={event => onServiceSelect(event, e.value)} />;
            })}
            </div>
            <div>
              <Status searchParams={_searchParams} businessServices={_searchParams.services} statusMap={statusMap || client.getQueryData(`INBOX_STATUS_MAP_${moduleCode}`)} moduleCode={moduleCode} onAssignmentChange={(e, status) => {
              if (e.target.checked) localParamChange({
