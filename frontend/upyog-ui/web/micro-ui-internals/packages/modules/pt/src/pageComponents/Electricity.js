...
const Electricity = ({
  ...
  useEffect(() => {
    if (userType === "employee") {
      ...
      if (electricity !== "undefined" && electricity?.length === 0) setFormError(config.key, {
        type: "required",
        message: t("CORE_COMMON_REQUIRED_ERRMSG")
      });else if (electricity !== "undefined" && electricity?.length < 10 || electricity?.length > 10 || !Number(electricity)) setFormError(config.key, {
        type: "invalid",
      ...
      return <React.Fragment>
          <LabelFieldPair key={index}>
            ...
            <div className="field">
              ...
              <TextInput key={input.name} id={input.name}
            //isMandatory={true}
            value={electricity} onChange={handleElectricityChange}
            //onChange={setElectricityNo}
            onSelect={goNext} placeholder={"Enter a valid 10-digit electricity number"} {...input.validation} onBlur={onBlur}
  ...
  return <React.Fragment>
      ...
      <FormStep config={config} onChange={handleElectricityChange} onSelect={goNext} onSkip={onSkip} t={t} isDisabled={electricity.length === 10 ? false : true} showErrorBelowChildren={true}>
        <CardLabel>{`${t("PT_ELECTRICITY")}`}<span className="check-page-link-button"> *</span></CardLabel>
        <TextInput t={t} type={"number"}
      //isMandatory="true"
      optionKey="i18nKey" name="electricity" value={electricity} onChange={handleElectricityChange} placeholder={"Enter a valid 10-digit electricity number"} {...validation = {
        required: true,
        minLength: 10,
        maxLength: 10
      }} />
