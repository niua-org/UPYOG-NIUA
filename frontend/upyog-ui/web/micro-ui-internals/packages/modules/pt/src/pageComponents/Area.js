...
const Area = ({
  ...
  const goNext = () => {
    if (!isNaN(index)) {
      ...
      if ((formData?.isResdential?.i18nKey === "PT_COMMON_YES" || formData?.usageCategoryMajor?.i18nKey == "PROPERTYTAX_BILLING_SLAB_NONRESIDENTIAL") && formData?.PropertyType?.i18nKey !== "COMMON_PROPTYPE_VACANT") {
    ...
    return inputs?.map((input, index) => {
      return <React.Fragment>
          <LabelFieldPair key={index}>
            ...
            <div className="field">
              <TextInput key={input.name} id={input.name} value={floorarea} onChange={onChange} {...input.validation} onBlur={onBlur}
            // autoFocus={presentInModifyApplication}
            />
            </div>
          </LabelFieldPair>
          {formState.touched[config.key] ? <CardLabelError className="pt-auto-36">
  ...
  return <React.Fragment>
    ...
    <FormStep config={config} onChange={onChange} forcedError={t(unitareaerror) || t(areanotzeroerror)} onSelect={goNext} onSkip={onSkip} t={t} isDisabled={unitareaerror || areanotzeroerror || !floorarea} showErrorBelowChildren={true}>
      <CardLabel>{`${t("PT_PLOT_SIZE_SQUARE_FEET_LABEL")}`}</CardLabel>
      <TextInput t={t} type={"number"} isMandatory={false} optionKey="i18nKey" name="floorarea" value={floorarea} onChange={setPropertyfloorarea} {...validation = {
        pattern: "^([0-9]){0,8}$",
        type: "number",
