...
const SelectOwnerShipDetails = ({
  ...
  useEffect(() => {
    if (!isLoading && SubOwnerShipCategoryOb && OwnerShipCategoryOb) {
      const preFilledPropertyType = SubOwnerShipCategoryOb.filter(ownershipCategory => ownershipCategory.code === (formData?.ownershipCategory?.value || formData?.ownershipCategory))[0];
  ...
  function getDropdwonForProperty(ownerShipdropDown) {
    if (userType === "employee") {
      const arr = ownerShipdropDown?.filter(e => e.code.split(".").length <= 2)?.splice(0, 10)?.map(ownerShipDetails => ({
        ...ownerShipDetails,
        i18nKey: `PT_OWNERSHIP_${ownerShipDetails.value.split(".")[1] ? ownerShipDetails.value.split(".")[1] : ownerShipDetails.value.split(".")[0]}`
      }));
      return arr;
    }
    return ownerShipdropDown && ownerShipdropDown.length && ownerShipdropDown.splice(0, 10).map(ownerShipDetails => ({
      ...ownerShipDetails,
      i18nKey: `PT_OWNERSHIP_${ownerShipDetails.value.split(".")[1] ? ownerShipDetails.value.split(".")[1] : ownerShipDetails.value.split(".")[0]}`
    }));
  }
  ...
  function goNext() {
    ...
    onSelect(config.key, ownershipCategory, "", index, null, {
      routeKey: ownershipCategory?.value
    });
  }
  useEffect(() => {
    if (userType === "employee") {
  ...
  return <React.Fragment>
      ...
      <FormStep t={t} config={config} onSelect={goNext} onSkip={onSkip} isDisabled={!ownershipCategory}>
        <div>
          <RadioButtons isMandatory={config.isMandatory} options={getDropdwonForProperty(ownerShipdropDown) || []} selectedOption={ownershipCategory} optionsKey="i18nKey" onSelect={selectedValue} value={ownershipCategory} labelKey="PT_OWNERSHIP" isDependent={true} disabled={isUpdateProperty || isEditProperty} />
        </div>
      </FormStep>
    </React.Fragment>;
};
