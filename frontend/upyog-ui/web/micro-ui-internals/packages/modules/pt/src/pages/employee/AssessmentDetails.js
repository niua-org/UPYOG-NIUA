...
const AssessmentDetails = () => {
  ...
  return <div>
      ...
      <ApplicationDetailsTemplate applicationDetails={{
      applicationDetails: [{
        title: "PT_ESTIMATE_DETAILS_HEADER",
        values: [{
          title: "PT_PROPERTY_PTUID",
        ...
        belowComponent: () => <LinkLabel onClick={() => {
          showPopUp(true);
        }} style={isMobile ? {
          ...
          color: "var(--primary-main)"
        }}>{t("PT_ADD_REBATE_PENALITY")}</LinkLabel>
      } : {}, {
        title: "PT_ASSESMENT_INFO_SUB_HEADER",
        values: [{
          title: "PT_ASSESMENT_INFO_TYPE_OF_BUILDING",
          value: getPropertyTypeLocale(applicationDetails?.applicationData?.propertyType)
        }, {
          title: "PT_ASSESMENT_INFO_USAGE_TYPE",
          value: getPropertySubtypeLocale(applicationDetails?.applicationData?.usageCategory)
        }, {
          title: "PT_ASSESMENT_INFO_PLOT_SIZE",
          value: applicationDetails?.applicationData?.landArea
        }, {
          title: "PT_ASSESMENT_INFO_NO_OF_FLOOR",
          value: applicationDetails?.applicationData?.noOfFloors
        }],
        additionalDetails: {
          floors: applicationDetails?.applicationData?.units?.filter(e => e.active)?.sort?.((a, b) => a.floorNo - b.floorNo)?.map((unit, index) => {
        ...
        belowComponent: () => {
          return <div className="pt-auto-154">
                  ...
                  <CardText className="pt-auto-157">{t("PT_CALC_LOGIC")}</CardText>
                    ...
                    <StatusTable>
                    {applicationDetails?.applicationData?.units?.filter(e => e.active)?.sort?.((a, b) => a.floorNo - b.floorNo)?.map((unit, index) => <Row label={`${t(`PROPERTYTAX_FLOOR_${unit?.floorNo}`)} ${t(`PT_UNIT`)} - ${index + 1}`} text={ChargeSlabsMenu?.PropertyTax && ChargeSlabsMenu?.PropertyTax?.ChargeSlabs?.filter(ob => ob.floorNo == unit.floorNo)?.[0]?.name} />)}
                    </StatusTable>
                   </div>
                  </div>;
        }
      }]
    }} showTimeLine={false} isLoading={isLoading} isDataLoading={isLoading} applicationData={appDetailsToShow?.applicationData} mutate={null} workflowDetails={queryClient.getQueryData(["PT_ASSESSMENT", propertyId, location?.state?.Assessment?.financialYear]) ? {
      ...workflowDetails,
      data: {
        ...workflowDetails.data,
        nextActions: []
      }
    } : workflowDetails} businessService="PT" assessmentMutate={assessmentMutate} ptCalculationEstimateMutate={ptCalculationEstimateMutate} showToast={showToast} setShowToast={setShowToast} closeToast={closeToast} timelineStatusPrefix={"ES_PT_COMMON_STATUS_"} forcedActionPrefix={"WF_EMPLOYEE_PT.CREATE"} />
      {/* {popup && (<RebatePenalityPoPup/>)} */}
      {popup && <Modal headerBarMain={<Heading label={t("PT_ADD_REBATE_PENALITY")} />} headerBarEnd={<CloseBtn onClick={() => {
      showPopUp(false), ptCalculationEstimateData.Calculation[0] = ptCalculationEstimateDataCopy;
      setSelectedPenalityReason(null);
      setSelectedRebateReason(null);
    }} />} actionCancelLabel={t("PT_CANCEL")} actionCancelOnSubmit={() => {
      ptCalculationEstimateData.Calculation[0] = ptCalculationEstimateDataCopy;
      setSelectedPenalityReason(null);
      setSelectedRebateReason(null);
      showPopUp(false);
    }} actionSaveLabel={t("PT_ADD")} actionSaveOnSubmit={() => change()} hideSubmit={false}>
      {<div>
        <Card>
        <CardSectionHeader>{t("PT_AD_PENALTY")}</CardSectionHeader>
            <CardLabel>
            ...
            <div className="field">
              <div className="field-container">
                <div className="text-input field">
                <Dropdown isMandatory option={Penality_menu} optionKey="value" select={selectPenalityReason} selected={selectedPenalityReason} isPropertyAssess={true} t={t} />
                </div>
              </div>
            </div>  
            {selectedPenalityReason && selectedPenalityReason.value === "Others" && <div className="field">
        ...
        <CardSectionHeader>{t("PT_AD_REBATE")}</CardSectionHeader>
            ...
            <div className="field">
              <div className="field-container">
                <div className="text-input field">
                <Dropdown isMandatory option={Rebate_menu} optionKey="value" select={selectRebateReason} selected={selectedRebateReason} isPropertyAssess={true} t={t} />
                </div>
              </div>
            </div>    
            {selectedRebateReason && selectedRebateReason.value === "Others" && <div className="field">
      ...
      {!queryClient.getQueryData(["PT_ASSESSMENT", propertyId, location?.state?.Assessment?.financialYear]) ? <ActionBar>
