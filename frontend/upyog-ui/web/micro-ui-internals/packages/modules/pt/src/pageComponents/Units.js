...
const Units = ({
  ...
  occupencyOptions = Menu?.PropertyTax?.OccupancyType?.map(e => ({
    i18nKey: `PROPERTYTAX_OCCUPANCYTYPE_${e?.code}`,
    ...e
  })) || [];
  let rentedmonths = [{
  ...
  function getfloorlistdata(floorlist) {
    floorListData = floorlist?.map(floor => ({
      i18nKey: "PROPERTYTAX_FLOOR_" + stringReplaceAll(floor?.code, "-", "_"),
      code: floor?.code
    })) || [];
  ...
  useEffect(() => {
    if (!isLoading && presentInModifyApplication && Menu) {
      ...
      let defaultUnits = formData?.originalData?.units?.filter(e => e.active)?.map((unit, index) => {
        ...
        return {
          ...
          NonRentedMonthsUsage: nonrentedusage.find(e => e.code === formData?.originalData?.additionalDetails?.unit?.[index]?.NonRentedMonthsUsage) || null,
          RentedMonths: rentedmonths.find(e => e.code === formData?.originalData?.additionalDetails?.unit?.[index]?.RentedMonths) || null,
          arv
        };
      });
  ...
  const calculateNumberOfFloors = () => {
    if (formData?.PropertyType?.code !== "BUILTUP.INDEPENDENTPROPERTY") {
      ...
      if (formData?.PropertyType?.code.includes("BUILTUP")) {
        ...
        let floorWiseAreas = units.reduce((acc, {
          ...
          if (isNaN(acc?.[floorNo?.code])) acc[floorNo?.code] = area;else acc[floorNo?.code] = acc[floorNo?.code] + area;
          return acc;
        }, {});
    ...
    if (unitsMissing.length && units.some(unit => unit?.floorNo?.code)) {
      setError(config.key, {
        type: "units_missing",
        message: `PT_FLOORS_MISSING_UNITS.${unitsMissing.map(e => e?.code).join()}`
      });
  ...
  const usageCategoryMajorMenu = usagecat => {
    const catMenu = usagecat?.filter(e => e?.code.split(".").length <= 2 && e?.code !== "NONRESIDENTIAL")?.map(item => {
  ...
  const subUsageCategoryMenu = category => {
    const menu = usagecat.filter(cat => cat?.code.includes(category?.code) && cat?.code.split(".").length === 4).map(item => {
  ...
  return formData?.PropertyType?.code === "VACANT" || !formData?.PropertyType?.code || !formData?.usageCategoryMajor?.code ? null : <div>
      {units?.map((unit, index) => <Unit t={t} key={unit.key} unit={unit} allUnits={units} setUnits={setUnits} index={index} stateId={stateId} focusIndex={focusIndex} setFocusIndex={setFocusIndex} isMobile={isMobile} getfloorlistdata={getfloorlistdata} floorlist={floorlist} occupencyOptions={occupencyOptions} rentedmonths={rentedmonths} ageOfPropertyOptions={ageOfPropertyOptions} structureTypeOptions={structureTypeOptions} nonrentedusage={nonrentedusage} formData={formData} handleRemoveUnit={handleRemoveUnit} {...{
      formState,
      setError,
      clearErrors,
...
function Unit({
  ...
  const [usageType, setUsageType] = useState(() => {
    ...
    if (formData?.usageCategoryMajor?.code !== "MIXED") {} else if (existingUsageCategory) {
  ...
  useEffect(() => {
    ...
    if (!_.isEqual(formValue, part)) setUnits(prev => prev.map(item => item.key === unit.key ? {
      ...item,
      ...formValue
    } : item));
    ...
    <div className="pt-auto-97">
      ...
      : null}
        ...
        <LabelFieldPair>
          ...
          <Dropdown className="form-field" selected={usageType} disable={formData?.usageCategoryMajor?.code !== "MIXED"} option={usageCategoryMajorMenu(usagecat)} select={setUsageType} optionKey="i18nKey" t={t} />
        </LabelFieldPair>

        <LabelFieldPair style={["RESIDENTIAL"].includes(usageType?.code) ? {
        display: "none"
      } : {}}>
