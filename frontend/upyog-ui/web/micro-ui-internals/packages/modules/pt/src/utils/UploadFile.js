...
const UploadFileDigiLocker = props => {
  ...
  } = Digit.Hooks.createTokenAPI("document");
  let extraStyles = {};
  const handleChange = () => {
  ...
  const fetchDigiLockerDocuments = async e => {
    ...
    const res1 = await Digit.DigiLockerService.issueDoc({
      TokenReq
    });
    ...
    console.log("url", uri);
    if (uri?.length > 0) {
      const res2 = await Digit.DigiLockerService.uri({
  ...
  const convertToFile = (e, blob) => {
    ...
    reader.onloadend = function () {
      var base64data = reader.result;
      var blobData = dataURItoBlob(base64data);
      let newFile = new File([blobData], `drivingL.pdf`, {
        type: "application/pdf"
      });
      props.onUpload(e, newFile);
    };
  };
  const Close = () => <svg width="24" height="24" viewBox="0 0 24 24" fill="#FFFFFF" xmlns="http://www.w3.org/2000/svg">
  ...
  return <Fragment>
      ...
      <div className={`upload-file ${user_type === "employee" ? "" : "upload-file-max-width"} ${props.disabled ? " disabled" : ""}`} style={extraStyles?.uploadFile ? extraStyles?.uploadFile : {}}>
        <div style={extraStyles ? extraStyles?.containerStyles : null}>
          ...
          {!hasFile || props.error ? <h2 className="file-upload-status">{props.message}</h2> : <div className="tag-container" style={extraStyles ? extraStyles?.tagContainerStyles : null}>
        ...
        <input className={props.disabled ? "disabled" : "" + "input-mirror-selector-button"} style={extraStyles ? {
        ...extraStyles?.inputStyles,
        ...props?.inputStyles
      } : {
        ...props?.inputStyles
      }} ref={inpRef} type="file" id={props.id || `document-${getRandomId()}`} name="file" multiple={props.multiple} accept={props.accept} disabled={props.disabled} onChange={e => props.onUpload(e)} onClick={event => {
        const {
          target = {}
        } = event || {};
        target.value = "";
