...
const SelectOwnerAddress = ({ t, config, onSelect, userType, formData, ownerIndex = 0 }) => {
  ...
  const [permanentAddress, setPermanentAddress] = useState(formData.owners && formData.owners[index] && formData.owners[index]?.permanentAddress || formData.owners[index]?.correspondenceAddress || formData?.owners?.permanentAddress || "");
  ...
  const checkMutatePT = window.location.href.includes("citizen/pt/property/property-mutation/") ? <Timeline currentStep={1} flow="PT_MUTATE" /> : <Timeline currentStep={2} />;
