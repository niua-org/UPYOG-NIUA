...
const PropertyStructureDetails = ({
  t,
  config,
  onSelect,
  userType,
  ...
  const [fields, setFields] = useState(window.location.pathname.includes("/pt/modify-application/") ? formData?.propertyStructureDetails : formData?.propertyStructureDetails ? formData?.propertyStructureDetails : window.location.pathname.includes("/edit-application/PropertyStructureDetails") ? {
    "usageCategory": "",
    "structureType": formData?.additionalDetails?.structureType,
    "ageOfProperty": formData?.additionalDetails?.ageOfProperty
  } : {
    "usageCategory": "",
    "structureType": "",
    "ageOfProperty": ""
  ...
  if (userType === "employee") {
    return <React.Fragment>
          ...
          <CardLabel>{`${t("PT_STRUCTURE_TYPE")}`}<span className="check-page-link-button"> *</span></CardLabel>
            <div className="field">
            <Dropdown t={t} optionKey="i18nKey" isMandatory={config.isMandatory} option={structureType} selected={fields?.structureType} placeholder={"Select structure type"} select={e => selectstructureType(1, e)} />
          ...
          <CardLabel>{`${t("PT_AGE_OF_PROPERTY")}`}<span className="check-page-link-button"> *</span></CardLabel>
            <div className="field">
            <Dropdown t={t} optionKey="i18nKey" isMandatory={config.isMandatory} option={ageOfProperty} selected={fields?.ageOfProperty} placeholder={"Select Age of Property"} select={e => selectageOfProperty(2, e)} />
    ...
    <FormStep config={(config.texts.header = getheader(), config)} onSelect={goNext} onSkip={onSkip} t={t} isDisabled={!fields.structureType || !fields.ageOfProperty}>
          ...
          <div key={`unique`}>
            <div className="pt-auto-73">
              ...
              <Dropdown t={t} optionKey="i18nKey" isMandatory={config.isMandatory} option={catMenu} selected={fields?.usageCategory} select={e => selectUsageCategory(0, e)} />
              </div> : ""}
              <CardLabel>{`${t("PT_STRUCTURE_TYPE")}`}<span className="check-page-link-button"> *</span></CardLabel>
