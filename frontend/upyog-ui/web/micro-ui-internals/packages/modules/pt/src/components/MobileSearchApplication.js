...
const MobileSearchApplication = ({
  ...
  const propsMobileInboxCards = useMemo(() => {
    ...
    return data?.map(data => ({
      ...
      [t("PT_ADDRESS_LABEL")]: `${data?.address?.doorNo ? `${data?.address?.doorNo}, ` : ""} ${data?.address?.street ? `${data?.address?.street}, ` : ""}${data?.address?.landmark ? `${data?.address?.landmark}, ` : ""}${t(data?.address?.locality.code)}, ${t(data?.address?.city.code)} ${t(data?.address?.pincode) ? `${data?.address.pincode}` : " "}` || "NA"
    }));
  }, [data]);
  return <React.Fragment>
      ...
      {currentlyActiveMobileModal ? <PopUp>
        ...
        onSubmit(data);
      }} handleSubmit={handleSubmit} id="search-form" className="rm-mb form-field-flex-one inboxPopupMobileWrapper" {...{
        searchFormFieldsComponentProps,
        currentlyActiveMobileModal,
        closeMobilePopupModal,
        tenantId
