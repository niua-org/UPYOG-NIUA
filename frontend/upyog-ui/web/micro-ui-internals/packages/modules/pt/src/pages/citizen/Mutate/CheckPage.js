...
const CheckPage = ({ onSubmit, value = {} }) => {
  ...
  return <React.Fragment>
    ...
    <StatusTable>
        ...
        <Row className="border-none" label={t("PT_SEARCHPROPERTY_TABEL_PTUID")} text={property?.propertyId} textStyle={{
          whiteSpace: "pre"
        }} />
        ...
        <Row className="border-none" label={t("PT_FEE_AMOUNT")} text={billAmount} textStyle={{
          whiteSpace: "pre"
        }} />
        <Row className="border-none" label={t("PT_PAYMENT_STATUS")} text={billStatus} textStyle={{
          whiteSpace: "pre"
        }} />
    </StatusTable>
    ...
    <CardSubHeader style={getCardSubHeadrStyles()}>{t("PT_MUTATION_TRANSFEROR_DETAILS")}</CardSubHeader>
      ...
      {ownershipCategory?.code?.includes("INSTITUTIONAL") ? <div>
            {Array.isArray(owners) && owners.map((transferorInstitution, index) => <div key={index}>
                    {owners.length != 1 && <span>
                        {t("PT_OWNER_SUB_HEADER")} - {index + 1}{" "}
                      </span>}
                  <StatusTable>
            ...
            {Array.isArray(Owners) && Owners.map((owner, index) => <div key={index}>
                    {Owners.length != 1 && <span>
                        {t("PT_OWNER_SUB_HEADER")} - {index + 1}{" "}
                      </span>}
                  <StatusTable>
                    ...
                    <Row className="border-none" label={t("PT_FORM3_OWNERSHIP_TYPE")} text={`${property?.ownershipCategoryTemp ? t(`PT_OWNERSHIP_${property?.ownershipCategoryTemp}`) : t("CS_NA")}`} />
                  </StatusTable>
                </div>)}
          </div>}
      ...
      <div>
        <StatusTable>
          <Row className="border-none" label={t("PT_MUTATION_PENDING_COURT")} text={t(additionalDetails?.isMutationInCourt?.code) || t("CS_NA")} />
          <Row className="border-none" label={t("PT_DETAILS_COURT_CASE")} text={additionalDetails?.caseDetails || t("CS_NA")} />
          <Row className="border-none" label={t("PT_PROP_UNDER_GOV_AQUISITION")} text={t(additionalDetails?.isPropertyUnderGovtPossession?.code) || t("CS_NA")} />
      ...
      <StatusTable>
        <Row className="border-none" label={t("PT_REASON_PROP_TRANSFER")} text={`${t(additionalDetails?.reasonForTransfer?.i18nKey)}` || t("CS_NA")} />
      ...
      <CheckBox label={t("PT_MUTATION_FINAL_DECLARATION_MESSAGE")} onChange={setdeclarationhandler} styles={{
        height: "auto",
        margin: '2rem 0'
      }} />
