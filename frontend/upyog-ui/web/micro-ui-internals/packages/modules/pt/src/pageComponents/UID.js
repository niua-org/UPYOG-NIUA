...
const UID = ({
  ...
  useEffect(() => {
    if (userType === "employee") {
      if (uid !== "undefined" && uid?.length === 0) setFormError(config.key, {
        type: "required",
        message: t("CORE_COMMON_REQUIRED_ERRMSG")
      });else if (uid !== "undefined" && (!/^[a-zA-Z0-9-]{0,15}$/.test(uid) || uid?.length !== 15)) setFormError(config.key, {
        type: "invalid",
    ...
    return inputs?.map((input, index) => {
      return <React.Fragment>
          <LabelFieldPair key={index}>
            ...
            value={uid} onChange={handleUIDChange}
            //onChange={setElectricityNo}
            onSelect={goNext} placeholder={"Enter a valid 15-digit alphanumeric characters UID"} {...input.validation} onBlur={onBlur}
  ...
  return <React.Fragment>
      ...
      <FormStep config={config} onChange={handleUIDChange} onSelect={goNext} onSkip={onSkip} t={t} isDisabled={uid.length === 15 ? false : true} showErrorBelowChildren={true}>
        <CardLabel>{`${t("PT_ELECTRICITY_UID")}`}<span className="check-page-link-button"> *</span></CardLabel>
        <TextInput t={t} type={"text"} isMandatory={false} optionKey="i18nKey" name="uid" value={uid} onChange={handleUIDChange} placeholder={"Enter a valid 15-digit alphanumeric characters UID"} {...inputs[0].validation}
