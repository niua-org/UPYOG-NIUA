...
const PropertyType = ({
  ...
  function getPropertyTypeMenu(proptype) {
    if (userType === "employee") {
      return proptype?.filter(e => e.code === "VACANT" || e.code.split(".").length > 1)?.map(item => ({
      ...
      if (Array.isArray(proptype) && proptype.length > 0) {
        for (i = 0; i < proptype.length; i++) {
          if (Array.isArray(proptype) && proptype.length > 0) menu.push({
  ...
  return <React.Fragment>
      ...
      //options={menu}
      options={getPropertyTypeMenu(proptype) || {}} selectedOption={BuildingType} onSelect={selectBuildingType} />
      </FormStep>
