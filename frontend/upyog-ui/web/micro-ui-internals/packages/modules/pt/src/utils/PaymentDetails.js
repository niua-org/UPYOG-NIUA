...
const printReciept = async (tenantId, payments) => {
  ...
  if (!payments?.fileStoreId) {
    ...
    response = await Digit.PaymentService.generatePdf(stateTenant, {
      Payments: [{
        ...payments
      }]
    }, "property-receipt");
  }
  const fileStore = await Digit.PaymentService.printReciept(stateTenant, {
...
const PaymentDetails = () => {
  ...
  useEffect(() => {
    if (data) {
      setPaymentObject(data?.Payments?.map(payment => {
        return {
