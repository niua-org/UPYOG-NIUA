...
const ADSDocumentDetails = ({
  ...
  return <div>
      ...
      {!isLoading ? <FormStep t={t} config={config} onSelect={handleSubmit} onSkip={onSkip} isDisabled={enableSubmit} onAdd={onAdd}>
        ...
        return <ADSSelectDocument key={index} document={document} t={t} error={error} setError={setError} setDocuments={setDocuments} documents={documents} setCheckRequiredFields={setCheckRequiredFields} />;
      })}
...
function ADSSelectDocument({
  ...
  } : doc?.dropdownData?.length > 0 ? doc?.dropdownData[0] : {});
  ...
  useEffect(() => {
    if (action === "update") {
      ...
      const docType = dropDownData.filter(e => e.code === originalDoc?.documentType).map(e => ({
        ...e,
