...
const ADSSearch = ({
  ...
  LocationDetails && LocationDetails.map(slot => {
    LocationData.push({
      i18nKey: `${slot.name}`,
      code: `${slot.code}`,
      value: `${slot.name}`
  ...
  const handleCartClick = () => {
    ...
    if (selectedCheckboxes.length === 0) {
      setShowToast({
        error: true,
        label: t("ADS_SELECT_AT_LEAST_ONE_SLOT")
      });
      return;
    }
    ...
    const duplicateSlot = selectedSlotDetails.some(selectedRow => cartSlotDetails.some(cartRow => cartRow.addType === selectedRow.addType && cartRow.location === selectedRow.location && cartRow.faceArea === selectedRow.faceArea && cartRow.nightLight === selectedRow.nightLight && cartRow.bookingDate === selectedRow.bookingDate));
    if (duplicateSlot) {
      setShowToast({
        error: true,
        label: t("ADS_ITEM_ALREADY_IN_CART")
      });
    ...
    for (let groupKey in groupedSlots) {
      ...
      for (let i = 1; i < sortedGroup.length; i++) {
        ...
        if (current.slotId - prev.slotId > 1) {
          setShowToast({
            error: true,
            label: t("ADS_SLOT_DATE_NOT_CONSECUTIVE")
          });
    ...
    setShowToast({
      success: true,
      label: `${selectedSlotDetails.length} item(s) added to cart.`
    });
  ...
  const handleViewCart = () => {
    if (cartDetails.length > 0) {
      ...
      setShowToast({
        error: true,
        label: t("ADS_NO_ITEMS_IN_CART")
      }); // Provide feedback if cart is empty
    }
  };
