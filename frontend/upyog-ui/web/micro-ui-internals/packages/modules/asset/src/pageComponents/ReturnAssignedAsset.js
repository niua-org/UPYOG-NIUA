import {
  ...
  return <React.Fragment>
        {returndetails.map((returndetails, index) => <OwnerForm key={returndetails.key} index={index} returndetails={returndetails} {...commonProps} />)}
        </React.Fragment>;
};
const OwnerForm = _props => {
  ...
  useEffect(() => {
    if (!_.isEqual(part, formValue)) {
      ...
      setreturndetails(prev => prev.map(o => o.key && o.key === returndetails.key ? {
        ...o,
  ...
  return <React.Fragment>
        <div className="asset-auto-215">
            ...
            {allAssets?.length > 2 ? <div className="asset-auto-217">
                ...
                <StatusTable>
                <Row label={t("ES_ASSET_RESPONSE_CREATE_LABEL")} text={`${t(checkForNA(applicationDetails?.applicationData?.applicationData?.applicationNo))}`} />
                </StatusTable>
                <StatusTable>
                <Row label={t("AST_ASSIGNED_USER")} text={`${t(checkForNA(applicationDetails?.applicationData?.applicationData?.assetAssignment?.assignedUserName))}`} />
                </StatusTable>
                <StatusTable>
                <Row label={t("AST_ASSIGNED_DATE")} text={`${t(checkForNA(formatDate(applicationDetails?.applicationData?.applicationData?.assetAssignment?.assignedDate)))}`} />
                </StatusTable>
                <StatusTable>
                <Row label={t("AST_DESIGNATION")} text={`${t("COMMON_MASTERS_DESIGNATION_" + checkForNA(applicationDetails?.applicationData?.applicationData?.assetAssignment?.designation))}`} />
                </StatusTable>
                <StatusTable>
                <Row label={t("AST_EMP_CODE")} text={`${t(checkForNA(applicationDetails?.applicationData?.applicationData?.assetAssignment?.employeeCode))}`} />
                </StatusTable>
                <LabelFieldPair>
