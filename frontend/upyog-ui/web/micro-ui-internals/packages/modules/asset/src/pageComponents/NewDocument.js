...
const NewDocument = ({ t, config, onSelect, formData }) => {
  ...
  return <div>
      ...
      {!isLoading ? <FormStep t={t} config={config} onSelect={handleSubmit} onSkip={onSkip} isDisabled={enableSubmit}>
          <CardLabelDesc>{t(`ASSET_UPLOAD_RESTRICTIONS_SIZE`)}</CardLabelDesc>
          {data?.ASSET?.Documents?.map((document, index) => <ASSETSelectDocument key={index} document={document} t={t} error={error} setError={setError} setDocuments={setDocuments} documents={documents} setCheckRequiredFields={setCheckRequiredFields} formData={formData} />)}
...
function ASSETSelectDocument({
  ...
  return <div className="asset-auto-208">
    ...
    {doc?.code === "OWNER.ASSETPHOTO" ? <div>
      ...
      <LabelFieldPair>
        <div className="field">
          <UploadFile onUpload={handleFileUpload} onDelete={() => {
          setUploadedFile(null);
          setLatitude(null);
          setLongitude(null);
