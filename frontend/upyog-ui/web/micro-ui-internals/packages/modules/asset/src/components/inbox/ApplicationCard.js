...
export const ApplicationCard = ({
  ...
  if (!data || data?.length === 0) {
    result = <Card className="asset-auto-25">
        {t("CS_MYAPPLICATIONS_NO_APPLICATION").split("\\n").map((text, index) => <p key={index} className="asset-auto-26">
              {text}
            </p>)}
      </Card>;
  } else if (data && data?.length > 0) {
    result = <DetailsCard data={data} serviceRequestIdKey={serviceRequestIdKey} linkPrefix={linkPrefix ? linkPrefix : "/upyog-ui/employee/asset/"} />;
  }
  return <React.Fragment>
      ...
      {popup && <PopUp>
          ...
          {type === "SEARCH" && <div className="popup-module">
              <SearchApplication type="mobile" onClose={handlePopupClose} onSearch={onSearch} isFstpOperator={isFstpOperator} searchParams={searchParams} searchFields={searchFields} />
            </div>}
        </PopUp>}
    </React.Fragment>;
};
