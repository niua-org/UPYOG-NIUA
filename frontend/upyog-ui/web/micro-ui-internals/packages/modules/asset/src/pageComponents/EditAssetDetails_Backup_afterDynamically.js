...
const OwnerForm = _props => {
  ...
  const cityResponseObject = Digit.Hooks.useCustomMDMSV2(tenantId, "ASSET", [{
  ...
  const stateResponseObject = Digit.Hooks.useCustomMDMSV2(stateTenantId, "ASSET", [{
    name: "AssetParentCategoryFields"
  }], {
    select: data => {
      const formattedData = data?.["ASSET"]?.["AssetParentCategoryFields"];
  ...
  return <React.Fragment>
            <div className="asset-auto-105">
                <div className="asset-auto-106">
                  ...
                  }} render={props => <TextInput value={props.value} type={"date"} disable={false} max={new Date().toISOString().split("T")[0]} autoFocus={focusIndex.index === editNewAssetDetails?.key && focusIndex.type === row.name} onChange={e => {
                    ...
                    required: t("CORE_COMMON_REQUIRED_ERRMSG"),
                    validDate: val => /^\d{4}-\d{2}-\d{2}$/.test(val) ? true : t("ERR_DEFAULT_INPUT_FIELD_MSG")
                  }} render={props => <TextInput value={props.value} type={row.type} disable={false} max={new Date().toISOString().split("T")[0]} autoFocus={focusIndex.index === editNewAssetDetails?.key && focusIndex.type === row.name} onChange={e => {
                    props.onChange(e.target.value);
                    setFocusIndex({
                      index: editNewAssetDetails.key,
                      type: row.name
