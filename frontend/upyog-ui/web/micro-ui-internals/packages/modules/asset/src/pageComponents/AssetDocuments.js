...
const AssetDocuments = ({
  ...
  const {
    isLoading,
    data
  } = Digit.Hooks.asset.useAssetDocumentsMDMS(stateId, "ASSET", "Documents");
  ...
  const goNext = () => {
    onSelect(config.key, {
      documents,
      AssetsDocumentLength: AssetsDocument?.length
    });
  };
  useEffect(() => {
    goNext();
  ...
  return <div>
      ...
      return <AssetSelectDocument key={index} document={document} action={action} t={t} id={`document-${index}`} error={error} setError={setError} setDocuments={setDocuments} accept=".pdf, .jpeg, .jpg, .png" documents={documents} formData={formData} setFormError={setFormError} clearFormErrors={clearFormErrors} config={config} formState={formState} />;
    })}
...
function AssetSelectDocument({
  ...
  return <div className="asset-auto-51">
      ...
      <LabelFieldPair>
        ...
        <div className="field">
          <UploadFile onUpload={selectfile} selectedDocument={comingDataFromAPI?.[0]?.fileStoreId} onDelete={() => {
          setUploadedFile(null);
        ...
        buttonType="button" error={!uploadedFile} />
        </div>
      </LabelFieldPair>
    </div>;
}
