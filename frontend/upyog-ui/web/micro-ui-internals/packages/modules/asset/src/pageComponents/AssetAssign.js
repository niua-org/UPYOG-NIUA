...
const AssetAssign = ({
  ...
  designationData && designationData.map(designation => {
    designationNamefromMDMS.push({
      i18nKey: `${designation.name}`,
  ...
  return <React.Fragment>
            {assigndetails.map((assigndetails, index) => <OwnerForm key={assigndetails.key} index={index} assigndetails={assigndetails} {...commonProps} />)}
        </React.Fragment>;
};
const OwnerForm = _props => {
  ...
  useEffect(() => {
    if (!_.isEqual(part, formValue)) {
      ...
      setassigndetails(prev => prev.map(o => o.key && o.key === assigndetails.key ? {
        ...o,
  ...
  return <React.Fragment>
            <div className="asset-auto-39">
                <div className="asset-auto-40">
                    ...
                    <StatusTable>
                        <Row label={t("AST_DEPARTMENT_LABEL")} text={`${t(checkForNA('COMMON_MASTERS_DEPARTMENT_' + applicationDetails?.applicationData?.applicationData?.department))}`} />
                    </StatusTable>
                    <LabelFieldPair>
