...
const ChangePasswordComponent = ({ config: propsConfig, t }) => {
  ...
  return (
    <Background>
      ...
      >
        <Header />
        <CardSubHeader className="text-center"> {propsConfig.texts.header} </CardSubHeader>
        <CardText>
          {`${t(`CS_LOGIN_OTP_TEXT`)} `}
          <b>
            {" "}
        ...
        {/* <div>
          <CardLabel className="mb-sm">{t("CORE_OTP_SENT_MESSAGE")}</CardLabel>
          <CardLabelDesc style={{ marginBottom: "0px" }}> {mobileNumber} </CardLabelDesc>
          <CardLabelDesc className="mb-sm"> {t("CORE_EMPLOYEE_OTP_CHECK_MESSAGE")}</CardLabelDesc>
        </div>
        <CardLabel className="mb-sm">{t("CORE_OTP_OTP")} *</CardLabel>
