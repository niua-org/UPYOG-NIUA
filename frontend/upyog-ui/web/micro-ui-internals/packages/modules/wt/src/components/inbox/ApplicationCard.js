...
export const ApplicationCard = ({
  ...
  if (!data || data?.length === 0) {
    result = <Card className="wt-auto-13">
        {t("CS_MYAPPLICATIONS_NO_APPLICATION").split("\\n").map((text, index) => <p key={index} className="wt-auto-14">
              {text}
            </p>)}
      </Card>;
  } else if (data && data?.length > 0) {
    result = <DetailsCard data={data} serviceRequestIdKey={serviceRequestIdKey} linkPrefix={linkPrefix ? linkPrefix : "/upyog-ui/employee/chb/"} />;
  }
