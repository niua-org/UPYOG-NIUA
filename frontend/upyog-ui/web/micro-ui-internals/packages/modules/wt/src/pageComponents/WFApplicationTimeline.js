...
const WFApplicationTimeline = props => {
  ...
  const businessService = bookingCode == "WT" ? "watertanker" : bookingCode === "MT" ? "mobileToilet" : bookingCode === "TP" ? "treePruning" : "unknown"; // for selecting the Module code from booking number
  ...
  const moduleName = bookingCode === "WT" ? "request-service.water_tanker" : bookingCode === "MT" ? "request-service.mobile_toilet" : bookingCode === "TP" ? "request-service.tree_pruning" : "request-service.unknown";
  const {
    isLoading,
    data
    ...
    switch (nextAction?.action) {
      case "PAY":
        return props?.userType === 'citizen' ? <div className="wt-auto-31">
            ...
            state: {
              tenantId: props.application.tenantId,
              bookingNo: props?.application?.bookingNo
            }
          }}>
  ...
  return <React.Fragment>
        {!isLoading && <Fragment>
            ...
            {data?.timeline && data?.timeline?.length === 1 ? <CheckPoint isCompleted={true} label={t(data?.timeline[0]?.state && `${data.timeline[0].state}` || "NA")} customChild={getTimelineCaptions(data?.timeline[0])} /> : <ConnectingCheckPoints>
                {data?.timeline && data?.timeline.map((checkpoint, index) => {
          return <React.Fragment key={index}>
                        <CheckPoint keyValue={index} isCompleted={index === 0} label={t(`${data?.processInstances[index].state?.["state"]}`)} customChild={getTimelineCaptions(checkpoint)} />
                      </React.Fragment>;
        })}
