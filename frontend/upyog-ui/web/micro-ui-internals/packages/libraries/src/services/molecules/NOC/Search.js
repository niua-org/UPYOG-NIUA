...
export const NOCSearch = {
  ...
  applicationDetails: async (t, tenantId, applicationNumber, userType) => {
    ...
    const buildingDetails = {
      ...
      values: [
        { title: "NOC_APP_NO_LABEL", value: <div><Link to={`/employee/obps/bpa/${bpaResponse?.[0]?.applicationNo}`}><span className="link" style={{color: "#a82227"}}>{bpaResponse?.[0]?.applicationNo}</span></Link></div> },
