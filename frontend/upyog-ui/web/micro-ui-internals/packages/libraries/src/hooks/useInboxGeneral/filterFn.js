export const filterFunctions = {
  ...
  EST: (filtersArg) => {
    ...
    const { applicationNumber, mobileNo,limit, offset, sortBy, sortOrder, total, applicationStatus, vendingType, vendingZone, status, services } = filtersArg || {};
    if (filtersArg?.applicationNo) {
      searchFilters.applicationNo = filtersArg?.applicationNumber;
    }
    if (applicationStatus && applicationStatus?.[0]) {
      workflowFilters.applicationStatus = applicationStatus.map((status) => status.code).join(",");
    }
