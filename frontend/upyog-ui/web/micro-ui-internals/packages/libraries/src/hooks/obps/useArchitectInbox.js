...
const useArchitectInbox = ({ tenantId, filters, withEDCRData = true, isTotalCount = false, config = {} }) => {
  if (!isTotalCount) {
    ...
    return queryTemplate({
      queryKey: ["INBOX_DATA", tenantId, ...Object.keys(_filters)?.map((e) => _filters?.[e])],
      queryFn: async () => {
      ...
      select: (data) => ({
        ...
        table: data?.items.map((application) => ({
          ...
          applicantName:
            application?.businessObject?.landInfo?.owners?.length == 1
              ? application?.businessObject?.landInfo?.owners?.[0]?.name
              : application?.businessObject?.landInfo?.owners?.filter((owner) => owner.isPrimaryOwner)?.[0]?.name,
